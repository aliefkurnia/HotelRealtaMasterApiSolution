...
{
    public static IQueryable<PurchaseOrderDetail> Search(this IQueryable<PurchaseOrderDetail> purchaseOrder, string keyword)
    {
        if (string.IsNullOrWhiteSpace(keyword))
            return purchaseOrder;
        ...
        return purchaseOrder.Where(p => 
            p.StockName.ToLower().Contains(lowerCaseKeyword)
            );
    }

    public static IQueryable<PurchaseOrderDetail> Sort(this IQueryable<PurchaseOrderDetail> purchaseOrder, string orderByQueryString)
    {
        if (string.IsNullOrWhiteSpace(orderByQueryString))
            return purchaseOrder.OrderBy(e => e.StockName);
